...
import { DAI, PYUSD, sUSDe, USDe, tBTC, wstETH, USDT, USDC, TUSD, cbETH, weETH, rsETH, ezETH, LBTC, rETH, solvBTC, stETH, WBTC, cbBTC, USD0, WETH, EURc, GHO, USDS, GNO, WLD, USDD, GHST, FDUSD } from "../addresses.js"
...
export const CHAINS_WITH_CHAINLINK_FEED_SUPPORT = typeSafeObjectKeys(NATIVE_PAIRS).map(Number)
...
export type AllowedQuotes<T extends ChainsWithChainLinkFeedSupport> = 
  T extends SupportedChain.Ethereum ? 'BTC' | 'ETH' | 'USD' :
  T extends SupportedChain.Base ? 'BTC' | 'ETH' | 'USD' :
  T extends SupportedChain.Sepolia ? 'ETH' | 'USD' :
  T extends SupportedChain.Gnosis ? 'ETH' | 'USD' :
  T extends SupportedChain.Optimism ? 'BTC' | 'ETH' | 'USD' :
  T extends SupportedChain.Arbitrum ? 'BTC' | 'ETH' | 'USD' :
  T extends SupportedChain.Polygon ? 'ETH' | 'USD' :
  T extends SupportedChain.Bsc ? 'BTC' | 'ETH' | 'USD' :
  never;
...
export const isExistBasePair = <T extends ChainsWithChainLinkFeedSupport>(
  ...
  invariant(
    CHAINS_WITH_CHAINLINK_FEED_SUPPORT.includes(chainId as any), 
  ...
  const invertedMatch = chainPairs.find(pair => 
    pair.includes(base as string) && pair.includes(quote as string)
  );
...
export const convertNameIntoDenominator = <T extends ChainsWithChainLinkFeedSupport>(
...
export const getFeedData = <T extends ChainsWithChainLinkFeedSupport>(
  ...
  for (const quoteFeed of quoteFeeds) {
    for (const baseFeed of baseFeeds) {
      if (quoteFeed === baseFeed) {
        return {
          feedIntermediaryDenominations: [convertNameIntoDenominator<T>(quoteFeed)],
  ...
  const getAllNatives = (): AllowedQuotes<T>[] => {
    ...
    NATIVE_PAIRS[chainId].forEach(pair => {
      const [base, quote] = pair.split('/') as [AllowedQuotes<T>, AllowedQuotes<T>];
      natives.add(base);
      natives.add(quote);
    });
  ...
  const tryPath = (path: AllowedQuotes<T>[]): { 
